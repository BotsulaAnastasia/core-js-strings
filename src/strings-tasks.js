...
function removeFirstOccurrences(str, value) {
  const valueInd = str.indexOf(value);
  switch (valueInd) {
    case 0:
      return str.slice(value.length);
    case -1:
      return str;
    case str.length - value.length:
      return str.slice(0, valueInd);
    default:
      return str.slice(0, valueInd) + str.slice(valueInd + value.length);
  }
}
...
function removeLastOccurrences(str, value) {
  const valueInd = str.lastIndexOf(value);
  switch (valueInd) {
    case 0:
      return str.slice(value.length);
    case -1:
...
function isPalindrome(str) {
  const origStr = str.toLowerCase().replace(/[^\w]|\s/g, '');
...
function findLongestWord(sentence) {
  ...
  for (let i = 1; i < wordsLength.length; i += 1) {
    if (wordsLength[maxLengthIndex] < wordsLength[i]) {
      maxLengthIndex = i;
    }
  }
